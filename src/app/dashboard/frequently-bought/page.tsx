...
export default function FrequentlyBoughtPage() {
  ...
  const handleSkuClick = useCallback((sku: string) => {
    setSearchTerm(sku);
    setActiveSearch(sku);
    setSelectedView('analysis'); // Switch to analysis view
  ...
  const handleRefresh = useCallback(() => {
    ...
    loadUniqueSkus();
    loadTopSkus();
  }, [activeSearch, getSelectedSubstoreValues, loadAnalysisData, loadUniqueSkus, loadTopSkus]);
  ...
  const handleSyncMappingConfirm = useCallback(async () => {
    try {
      ...
      setSyncProgress({
        show: true,
        processed: 0,
        total: 1000, // Estimate, will update as we go
        percentage: 0,
        currentBatch: null,
        ...
        try {
          ...
          if (result.success) {
            ...
            setSyncProgress(prev => {
              ...
              // If we got a full batch, estimate we'll get more (conservative estimate)
              // If we got less than full batch, we're likely near the end
              let estimatedTotal: number;
              let percentage: number;
              
              if (result.progress.processed === batchSize) {
                // Full batch - estimate conservatively (assume at least 1 more batch)
                // Use a more realistic estimate based on current rate
                estimatedTotal = Math.max(newProcessed + batchSize, totalSynced + batchSize);
                percentage = Math.round((newProcessed / estimatedTotal) * 100);
                // Allow up to 97% during full batch processing
                percentage = Math.min(percentage, 97);
              } else if (result.progress.processed > 0) {
                // Partial batch - we're likely near the end
                // Estimate total more accurately: current + small buffer
                // This makes the progress feel more real as we approach completion
                const remainingEstimate = Math.max(
                  Math.ceil(result.progress.processed * 0.2), // Very conservative - likely just a bit more
                  10 // Minimum buffer
                );
                estimatedTotal = newProcessed + remainingEstimate;
                percentage = Math.round((newProcessed / estimatedTotal) * 100);
                // Can go up to 99% when getting partial batches (near completion)
                percentage = Math.min(percentage, 99);
              } else {
                // No products - keep previous estimate
                estimatedTotal = prev.total;
      ...
      setSyncProgress(prev => {
        if (!prev) return null;
        // First update to show we're completing
        const finalTotal = Math.max(totalSynced, prev.processed);
        return {
          ...prev,
          processed: totalSynced,
          total: finalTotal,
  ...
  const handleManualSkusConfirm = useCallback((skus: string[]) => {
    // Store manual SKUs
    setManualSkus(skus);
    // Close manual dialog
  ...
  const handlePushAllUpdatesConfirm = useCallback(async () => {
    try {
      ...
      const initialLogs = [
        ...
        ...(manualSkus.length > 0 
          ? [`📝 Manual SKUs configured: ${manualSkus.length} SKU${manualSkus.length > 1 ? 's' : ''} (${manualSkus.join(', ')})`]
          : []
        ),
      ...
      const batchLogMessage = manualSkus.length > 0
        ? `Starting batch processing with ${manualSkus.length} manual SKU${manualSkus.length > 1 ? 's' : ''} (${manualSkus.join(', ')}) - these will be merged with auto-found products`
  ...
  }, [manualSkus]); // Include manualSkus in dependency array
  ...
  const handlePushSingleUpdateConfirm = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('/api/frequently-bought/push-single', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sku: pushSingleSku, limit: 6, manualSkus }),
      });
      const result = await response.json();
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        {/* Info Banner */}
        {!activeSearch && selectedView === 'analysis' && analysisData.length === 0 && (
        ...
        {selectedView === 'analysis' ? (
          ...
          <AllSkusView 
            uniqueSkus={topSkus} 
            loading={loading || (topSkus.length === 0 && !activeSearch)}
            loadingAnalysis={loadingAnalysis}
          />
        )}
      </main>
      ...
      <ConfirmationModal
        ...
        message={`This will push frequently bought together data for ALL SKUs to Urvann API${manualSkus.length > 0 ? ` (with ${manualSkus.length} manual SKUs: ${manualSkus.join(', ')})` : ''}. This may take several minutes. Continue?`}
        confirmText="OK"
        cancelText="Cancel"
      ...
      <ConfirmationModal
        ...
        message={pushSingleSku ? `Push frequently bought together data for SKU: ${pushSingleSku}${manualSkus.length > 0 ? ` (with ${manualSkus.length} manual SKUs: ${manualSkus.join(', ')})` : ''}?` : ''}
        confirmText="OK"
        cancelText="Cancel"
