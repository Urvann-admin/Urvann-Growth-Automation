...
function CustomSelect<T extends string>({
  ...
  const triggerBorder = hasError ? 'border-red-300 focus:ring-red-500' : 'border-slate-300 focus:ring-amber-500 focus:border-amber-500';
  const isSm = size === 'sm';
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className={`${triggerHeight} w-full rounded-lg border bg-white ${triggerPadding} text-left ${textSize} text-slate-900 shadow-sm transition-shadow focus:outline-none focus:ring-2 focus:border ${triggerBorder} flex items-center justify-between gap-2`}
      >
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full min-w-[140px] rounded-lg border border-slate-200 bg-white py-1 shadow-lg">
          {options.map((opt) => (
            <button
              key={opt.value}
              type="button"
              onClick={() => { onChange(opt.value); setOpen(false); }}
              className={`w-full ${optionPadding} text-left ${textSize} transition-colors first:rounded-t-md last:rounded-b-md ${
                opt.value === value
                  ? 'bg-violet-600 text-white'
                  : 'text-slate-800 hover:bg-violet-600 hover:text-white'
              }`}
            >
              {opt.label}
            </button>
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="h-10 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-left text-sm text-slate-900 shadow-sm transition-shadow focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 flex items-center justify-between gap-2"
      >
        <span className={selected ? '' : 'text-slate-400'}>{selected ? selected.label : placeholder || 'Select...'}</span>
        <ChevronDown className={`w-4 h-4 shrink-0 text-slate-500 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
        <div className="absolute z-50 mt-1 w-full min-w-[180px] rounded-lg border border-slate-200 bg-white shadow-lg overflow-hidden">
          <div className="shrink-0 border-b border-slate-200 p-2">
            <input
              ...
              className="h-9 w-full rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
            />
          </div>
              ...
              filtered.map((opt) => (
                <button
                  ...
                  className={`w-full px-3 py-2 text-left text-sm transition-colors ${
                    opt.value === value ? 'bg-violet-600 text-white' : 'text-slate-800 hover:bg-violet-600 hover:text-white'
                  }`}
                >
                  {opt.label}
                </button>
  ...
  return (
    <>
      <button
        ...
        className={`h-10 w-full rounded-lg border bg-white px-3 py-2 text-left text-sm text-slate-900 shadow-sm transition-shadow focus:outline-none focus:ring-2 focus:border ${triggerBorder} flex items-center justify-between gap-2`}
      >
      ...
      {open && (
        ...
        >
          <div
            ...
            aria-hidden
          />
          <div className="relative w-full max-w-md max-h-[85vh] flex flex-col rounded-xl border border-slate-200 bg-white shadow-xl">
            ...
            <div className="overflow-auto py-2 max-h-[50vh]">
              {filtered.length === 0 ? (
                ...
                filtered.map((opt) => (
                  <label
                    key={opt.value}
                    className="flex cursor-pointer items-center gap-3 px-4 py-2.5 text-sm text-slate-800 hover:bg-slate-50"
                  >
                    <input
                      type="checkbox"
            ...
            <div className="shrink-0 flex justify-end gap-2 border-t border-slate-200 px-4 py-3">
              <button
                type="button"
                onClick={() => setOpen(false)}
...
const inputBase =
  'h-10 w-full rounded-lg border bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-shadow';
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">All fields marked with * are required.</p>
      </div>

      {/* Required fields – compact grid */}
      <div className="mb-6">
        <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Required fields</h3>
      ...
      {/* Hierarchy – optional */}
      <div className="mb-6">
        <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Hierarchy <span className="font-normal text-slate-400">(optional)</span></h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Type & rule</h3>
          <div className="flex flex-col md:flex-row gap-4 md:gap-5">
            ...
            {type === 'Automatic' && (
              <div className="flex-1 min-w-0 rounded-lg border border-slate-200 bg-slate-50/50 p-4 space-y-3">
                <div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button type="button" onClick={addCondition} className="inline-flex items-center gap-1 text-sm font-medium text-emerald-600 hover:text-emerald-700">
                    ...
                    {conditions.map((cond, index) => (
                      <div key={index} className="flex flex-wrap items-center gap-2">
                        ...
                        <input
                          ...
                          className="h-9 rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-sm w-28 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                        />
                        <button
                          type="button"
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-4">Publish & order</h3>
          <div className="flex flex-wrap items-center gap-6">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-sm font-medium text-slate-700">Publish <span className="text-red-500">*</span></span>
            </label>
            <div className="flex items-center gap-3">
              ...
              <div className="inline-flex overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
                <button
                  ...
                  className="flex h-8 w-8 shrink-0 items-center justify-center bg-slate-200 text-slate-700 transition-colors hover:bg-slate-300 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-1"
                ...
                <input
                  ...
                  className={`h-8 w-10 shrink-0 border-0 bg-white text-center text-xs font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-inset [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none ${errors.priorityOrder ? 'ring-2 ring-red-400 ring-inset' : ''}`}
                />
                <div className="w-px self-stretch bg-slate-200 shrink-0" aria-hidden />
                <button
                  type="button"
          ...
          {substores.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {substores.map((sub) => (
                <span
                  key={sub}
                  className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-sm text-slate-800"
                >
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 pt-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 min-w-[100px]"
          >
            Cancel
          </button>
        </div>
